/**
 * Safaricom Daraja wire contracts (verified against the consolidated Daraja knowledge
 * base captured 2026-09-13).
 *
 * These types mirror the provider's payloads exactly, including its spellings — Daraja
 * really does send `Occassion` on B2C. Correcting a provider's typo in our own types is
 * how integrations silently stop working, so the misspellings are preserved and
 * commented rather than tidied.
 */
...
/** Gateway-level error body: { requestId, errorCode, errorMessage }. */
 ...
 * results and a bare object when there is exactly one, and `ReferenceData.ReferenceItem`
