 ...
 *    promise, so a burst of queued payments produces one token request, not one per
 *    payment. The API layer keeps ONE client per enabled configuration (keyed by config
 *    id) so this cache actually earns its keep — a fresh client per request would double
 *    the provider call volume and court spike-arrest (500.003.02).
 *  - **No silent retries on payment submission.** A retried B2C request whose response we
 *    did not see risks paying twice. The client retries the *token* call and read-only
 *    queries only — never the payment call. Ambiguity is escalated to reconciliation.
 *  - **Timeouts.** Every request is bounded by an AbortController. A worker hanging on a
 *    provider socket holds a payment in limbo, which is worse than a clean timeout.
 *  - **Errors carry no secrets.** Request bodies are never echoed into error messages;
 *    the credential fields would travel with them.
 */
...
export class DarajaClient {
  ...
  async getAccessToken(forceRefresh = false): Promise<string> {
    ...
    // issue 200 token requests and trip Daraja's spike-arrest.
  ...
  /** Read-only balance query (executive balance panel). */
