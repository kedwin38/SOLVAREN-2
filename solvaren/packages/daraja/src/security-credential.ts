 ...
 *
 * Safaricom specifies: take the initiator password (base64-encoded), encrypt it with
 * M-PESA's public key (an X.509 certificate) using **RSA with PKCS #1 v1.5 padding —
 * explicitly not OAEP** — and base64-encode the result.
 *
 * Node's WebCrypto deliberately does not implement RSAES-PKCS1-v1_5 *encryption* (it is
 * legacy and vulnerable to Bleichenbacher padding-oracle attacks against a *decrypting*
 * party). We are the encrypting party and the scheme is mandated by the provider, so the
 * operation is implemented directly:
 *
 *   - the padding is built per RFC 8017 §7.2.1 (EME-PKCS1-v1_5), with the non-zero
 *     padding bytes drawn from `crypto.getRandomValues`, never from `Math.random`;
 *   - the modular exponentiation is plain BigInt `m^e mod n` over public key material,
 *     so there is no private-key timing surface to protect;
 *   - the certificate is parsed with a minimal DER walker rather than a dependency, so
 *     the package carries no ASN.1 library.
 *
 * The implementation's output is verified against OpenSSL decryption in the test suite,
 * including keys parsed from a genuine X.509 certificate structure.
 *
 * Operators who prefer not to hold the initiator password in the platform at all can
 * paste a credential generated offline on the Daraja portal; `isPrecomputedCredential`
 * recognises that case and the client uses the stored value as-is. Either way the
...
function readDer(buf: Uint8Array, offset: number): DerNode {
  ...
  const first = buf[i];
  if (first === undefined) {
    throw validationError('DARAJA_CERT_INVALID', 'Malformed certificate: truncated header');
  }
  let length: number;
  if (first < 0x80) {
    length = first;
    ...
    length = 0;
    for (let k = 1; k <= byteCount; k++) length = length * 256 + (buf[i + k] ?? 0);
    i += byteCount + 1;
  }
  const start = i;
...
export function extractRsaPublicKey(pemOrDer: string | Uint8Array): RsaPublicKey {
  ...
  // Cases 2 & 3: walk the structure looking for the BIT STRING that wraps the public key.
      ...
      if (node.tag === TAG_BIT_STRING) {
        ...
        if (der[node.start] === 0x00) {
          try {
            ...
            // Not the key — keep looking (certificates contain other BIT STRINGs, such
            // as the signature).
          }
        }
      }
 ...
 *
 * The result may be reused across requests (Safaricom explicitly permits this), but a
 * fresh value is generated on each rotation because the padding is randomised — two
 * encryptions of the same password differ, which is a useful property when proving a
