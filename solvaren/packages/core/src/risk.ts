 ...
 *
 * Decision model: evidence → deterministic policy checks → (optional) statistical/AI
 * analysis → risk score + reasons + evidence → human review → audited action. This module
 * is the deterministic half — the half that can *block*. The AI layer may add narrative
 * and soft signals; it cannot clear a finding raised here (AI CONTROL).
 *
 * Every signal is explainable: it carries the evidence that produced it, because "risk
 * score 78" is not something a finance officer can act on. All nine spec §11.1 signal
 * families are implemented, including the department/payroll variance and payment
 * frequency signals.
 */
...
export interface RecipientHistory {
  ...
  paymentCount: number;
  /** Median hours between this recipient's prior successful payments; null if <2. */
...
export interface DepartmentBaseline {
  departmentId: string;
  /** Mean total disbursed to this department per settled batch over the trailing window. */
  meanBatchTotalCents: number;
...
export interface RiskInput {
  ...
  /** Per-department trailing baselines, for the department variance signal. */
  departmentBaselines: ReadonlyMap<string, DepartmentBaseline>;
  /** Current per-department totals within this batch (computed by the caller). */
  departmentTotalsCents: ReadonlyMap<string, number>;
  now: number;
...
export interface RiskPolicy {
  ...
  /** A recipient record modified within this many hours is "recently modified". */
  recipientModifiedWindowHours: number;
  /** Batch total deviating from the trailing mean by more than this fraction is flagged. */
  batchTotalDeviationFraction: number;
  /** Department total deviating by more than this fraction is flagged. */
  departmentDeviationFraction: number;
  /** Payments to one recipient within this many hours are "unusual frequency". */
  frequencyWindowHours: number;
  /** Edits within this many minutes of submission are "late". */
  lateEditWindowMinutes: number;
  /** Share of amounts that are round to trigger the round-number cluster signal. */
  ...
  /** Business hours in East Africa Time, as minutes of day. */
  businessHoursEat: { startMinute: number; endMinute: number };
}

export const DEFAULT_RISK_POLICY: RiskPolicy = {
  amountDeviationMultiple: 2.5,
  newRecipientWindowHours: 72,
  recipientModifiedWindowHours: 72,
  batchTotalDeviationFraction: 0.25,
  departmentDeviationFraction: 0.4,
  frequencyWindowHours: 24,
  lateEditWindowMinutes: 10,
  roundNumberThreshold: 0.9,
  acknowledgementThreshold: 40,
  businessHoursEat: { startMinute: 7 * 60, endMinute: 19 * 60 }, // 07:00–19:00 EAT
};
...
export function assessBatchRisk(input: RiskInput): RiskAssessment {
  ...
  for (const [recipientId, group] of byRecipient) {
    if (group.length > 1 && new Set(group.map((g) => g.amountCents)).size > 1) {
      signals.push({
        ...
        evidence: { recipientId, amounts: group.map((g) => formatCents(g.amountCents)) },
        instructionIds: group.map((g) => g.instructionId),
      });
    }
  }

  // ---- Per-recipient: novelty, deviation, modification, frequency ---------
  for (const i of instructions) {
    ...
    if (modifiedHoursAgo <= policy.recipientModifiedWindowHours && h.paymentCount > 0) {
      signals.push({
    ...
    // Frequency: another payment to this recipient inside the window (history says one
    // was paid recently, or this batch itself pays them and they were paid very recently).
    if (h.lastPaidAt !== null) {
      const hoursSinceLast = (now - h.lastPaidAt) / HOUR_MS;
      const group = byRecipient.get(i.recipientId)!;
      const inBatchAgain = group.length > 1;
      if (hoursSinceLast < policy.frequencyWindowHours && (h.paymentCount > 2 || inBatchAgain)) {
        signals.push({
          type: 'UNUSUAL_FREQUENCY',
          severity: hoursSinceLast < 6 ? 'HIGH' : 'MEDIUM',
          summary: `This recipient was paid ${Math.round(hoursSinceLast)} hours ago; paying them again now is unusual for this cycle.`,
          evidence: {
            recipientId: i.recipientId,
            hoursSinceLastPayment: Number(hoursSinceLast.toFixed(1)),
            priorPayments: h.paymentCount,
            occurrencesInThisBatch: group.length,
          },
          instructionIds: group.map((g) => g.instructionId),
        });
      }
  ...
  // ---- Department / payroll variance ---------------------------------------
  for (const [departmentId, currentTotal] of input.departmentTotalsCents) {
    const baseline = input.departmentBaselines.get(departmentId);
    if (!baseline || baseline.meanBatchTotalCents <= 0 || baseline.settledBatchCount < 2) continue;
    const delta = Math.abs(currentTotal - baseline.meanBatchTotalCents) / baseline.meanBatchTotalCents;
    if (delta >= policy.departmentDeviationFraction) {
      const direction = currentTotal > baseline.meanBatchTotalCents ? 'higher' : 'lower';
      signals.push({
        type: 'DEPARTMENT_VARIANCE',
        severity: delta >= policy.departmentDeviationFraction * 2 ? 'HIGH' : 'MEDIUM',
        summary: `This department's total of KES ${formatCents(currentTotal)} is ${Math.round(delta * 100)}% ${direction} than its recent per-batch average of KES ${formatCents(Math.round(baseline.meanBatchTotalCents))}.`,
        evidence: {
          departmentId,
          currentTotal: formatCents(currentTotal),
          recentAverage: formatCents(Math.round(baseline.meanBatchTotalCents)),
          deviationPercent: Math.round(delta * 100),
          comparedBatches: baseline.settledBatchCount,
        },
  ...
  // ---- Timing (East Africa Time; input.now is UTC epoch ms) ----------------
  const eatMinutes = new Date(now + 3 * HOUR_MS).getUTCHours() * 60 + new Date(now + 3 * HOUR_MS).getUTCMinutes();
  if (eatMinutes < policy.businessHoursEat.startMinute || eatMinutes >= policy.businessHoursEat.endMinute) {
    signals.push({
      type: 'UNUSUAL_TIMING',
      severity: 'LOW',
      summary: "This batch is being processed outside the organization's normal business hours.",
      evidence: { eatMinutesOfDay: eatMinutes, businessHoursEat: policy.businessHoursEat },
      instructionIds: [],
    });
  }

  // ---- Round-number cluster -------------------------------------------------
  const round = instructions.filter(
    (i) => i.amountCents % 1_000_00 === 0 || i.amountCents % 500_000 === 0,
  ).length;
  if (instructions.length >= 5 && round / instructions.length >= policy.roundNumberThreshold) {
    signals.push({
      type: 'ROUND_NUMBER_CLUSTER',
      severity: 'LOW',
      summary: `${Math.round((round / instructions.length) * 100)}% of the amounts in this batch are round figures (whole KES 1,000s), which is unusual for real payroll data.`,
      evidence: {
        roundAmounts: round,
        totalInstructions: instructions.length,
        share: Number((round / instructions.length).toFixed(2)),
      },
      instructionIds: [],
    });
  }
  ...
  // ---- Score -----------------------------------------------------------------
  // Diminishing returns per signal type: fifty "new recipient" rows in a genuine
  // onboarding batch must not score the same as fifty distinct anomaly classes.
