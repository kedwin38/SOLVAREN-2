/**
 * Separation of duties and collusion controls (spec §20).
 *
 * These rules are the reason a single compromised account cannot move money. Each returns
 * a *reason* rather than a bare boolean so the denial can be shown to the user and written
 * to the audit log. They are additionally enforced as CHECK constraints in the database —
...
/**
 * Cooling-off: a material edit immediately before submission is the classic "approve the
 * clean version, pay the dirty one" pattern. The window is organization policy; zero
 * disables it.
 */
export function assertCoolingOff(params: {
  ...
  if (elapsed < coolingOffSeconds) {
    ...
    throw policyError(
      'SOD_COOLING_OFF',
      `This batch was edited ${Math.floor(elapsed)} seconds ago. Organization policy requires a ` +
        `${coolingOffSeconds}-second cooling-off period before submission; ${remaining} seconds remain.`,
      { remainingSeconds: remaining, coolingOffSeconds },
    );
  }
}

/** A conflict-of-interest registration for an approver/authorizer. */
...
/**
 * The same small group approving and authorizing together, repeatedly. This does not
 * block anything — it raises a risk finding for human review, because the legitimate
 * version (a small finance team) and the illegitimate version look identical from inside
 * the data.
 */
  ...
  for (const [pair, occurrences] of counts) {
    ...
    if (share >= concentrationThreshold) {
      signals.push({
        ...
        shareOfRecent: share,
        reason:
          `${Math.round(share * 100)}% of the last ${recentPairs.length} payment releases were approved ` +
          'and authorized by the same pair of officers. Consider widening the approval rota.',
      });
    }
  }
  return signals;
