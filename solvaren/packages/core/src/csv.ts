/**
 * CSV ingestion for payment batches (spec §6.1).
 *
 ...
 *  - A *formula* in the amount column is rejected outright; neutralisation happens on
 *    export, not here.
 *  - Every failing row produces a row-level reason with a 1-based line number, so the
 *    operator fixes the file rather than guessing (§23: "show field-level errors").
 *  - Intra-file duplicates are warned, not rejected — the reviewer decides.
 */
...
/** Hard ceiling on a single upload; larger payrolls split into multiple batches. */
...
export interface CsvParseOptions {
  maxRows?: number;
  /** Whole-shilling enforcement is a documented policy choice (see money.ts). */
  requireWholeShillings?: boolean;
}
...
export function parsePaymentCsv(text: string, options: CsvParseOptions = {}): CsvParseResult {
  ...
  const requireWhole = options.requireWholeShillings ?? true;
  ...
  dataRows.forEach((cells, index) => {
    const lineNumber = index + 2; // +1 zero-index, +1 header line
    ...
    const phoneCheck = tryNormalizeMsisdn(phoneRaw);
    if (!phoneCheck.ok) {
      errors.push({
        lineNumber,
        column: 'msisdn',
        value: phoneRaw,
        reason: phoneCheck.reason ?? 'Invalid phone number',
      });
      rowFailed = true;
    }
    ...
    if (FORMULA_PREFIX.test(amountRaw)) {
      ...
      try {
        ...
        if (amountCents <= 0) {
          ...
          rowFailed = true;
        } else if (requireWhole && amountCents % 100 !== 0) {
          errors.push({
            lineNumber,
            column: 'amount',
            value: amountRaw,
            reason: 'Payroll disbursements are configured for whole shillings; remove the cents component',
          });
          rowFailed = true;
        ...
        const message = err instanceof SolvarenError || err instanceof Error ? err.message : 'Amount is not a valid number';
    ...
    rows.push({
      lineNumber,
      recipientName: name,
      msisdn: phoneCheck.ok ? phoneCheck.msisdn : phoneRaw,
      amountCents,
  ...
  // Intra-file duplicate detection: same number, same amount, twice in one payroll run
  // is nearly always a copy-paste accident — and the single most expensive one.
