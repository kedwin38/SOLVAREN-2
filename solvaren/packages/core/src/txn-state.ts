 ...
 *     reconciliation result that disagrees raises a discrepancy case instead (§14:
 *     historical transaction records are not alterable).
...
/** Statuses shown as "still moving"; they drive the reconciliation sweep. */
...
/** Statuses eligible for the failed-transactions export by default. */
export const FAILED_EXPORT_STATES: readonly TxnState[] = ['FAILED'];
/** Optional additions to that export, per the "include ambiguous" toggle. */
...
export interface TxnTransitionInput {
  ...
  /** Provider failure code — required to reach FAILED. */
...
export function assertTxnTransition(input: TxnTransitionInput): TxnTransitionResult {
  ...
  if (from === to) {
    ...
    // records the duplicate as evidence and leaves the ledger untouched.
  ...
  if (to === 'SUCCESS') {
    // Evidence, not assertion (§24: "attempt to forge a SUCCESS status … confirm rejected").
    if (source === 'SYSTEM') {
      throw stateError(
...
/** Colour semantics for the explorer's status chips — never the only signal (a11y). */
...
/**
 * Whether a failed transaction may be retried. Never a blind resend of the same
 * instruction: §9.3 forbids retrying into an ambiguous outcome, and codes like 2001
 * (bad initiator credentials) fail identically until a human acts. A retry always
 * creates a NEW instruction through the audited retry workflow.
 */
