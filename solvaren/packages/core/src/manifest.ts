/**
 * Transaction manifest, authorization challenge and FPAC binding (spec §7.4–7.6).
 *
 ...
 * Any change to the amount, the recipient set, the approval chain or the policy context
 * produces a different digest, which invalidates the authorization already in flight and
 * forces a new approval path (§24: "change a payment amount after Level 3 review and
 * confirm the prior signature becomes invalid").
 *
 * Determinism rules (all load-bearing — changing any is a breaking protocol change):
 ...
 *   - Fields are joined with ASCII unit separators that cannot occur in the values.
...
export const MANIFEST_VERSION = 'SLV-MANIFEST-2' as const;
...
export interface ManifestInput {
  ...
  approvalId: string;
  /** Version of the batch the approver actually approved. */
  approvedBatchVersion: number;
  /** Hash of the organization policy in force — policy changes also invalidate. */
  policyDigest: string;
...
export interface ChallengeInput {
  ...
  expiresAt: number;
  /** The L3 user the challenge was issued to — not transferable. */
...
export interface AuthorizationChallenge {
  ...
  authorizerUserId: string;
  batchId: string;
  /** What the authorizer is shown and must confirm before signing (NFR-UX). */
  display: {
    batchReference: string;
    recipientCount: number;
    totalAmountCents: number;
...
/** Default challenge lifetime — short enough to bound replay, long enough to read. */
...
/**
 * Final gate before release. Every check has a corresponding attack:
 ...
 *   - policy digest→ tightening limits between the open and release steps
 */
export function verifyChallengeBinding(input: ChallengeVerificationInput): void {
  ...
  if (stored.authorizerUserId !== presentingUserId) {
    throw stateError('CHALLENGE_ACTOR_MISMATCH', 'This authorization challenge was issued to a different user');
  }
  ...
  if (current.approvedBatchVersion !== current.batchVersion) {
    throw stateError('MANIFEST_APPROVAL_STALE', 'The approval bound to this batch no longer matches its version');
  }
}
