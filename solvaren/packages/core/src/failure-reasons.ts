/**
 * Daraja failure-reason dictionary (spec §6, §23; verified against the consolidated
 * Daraja knowledge base captured 2026-09-13).
 *
 * Every code Safaricom documents is mapped to an explanation an operations officer can
 * act on, plus an `operatorAction` telling them what to actually do. Three rules:
 *
 *   1. Never blank. An unmapped code falls through to the provider's own description.
 *   2. Never just "Error". With nothing at all, say so explicitly and show the code.
 *   3. Versioned and data-driven. The table seeds `failure_reason_map` so new provider
 *      codes are mappable by an administrator without a deploy; the compiled table is
 *      the fallback when the database has no row.
 *
...
export const FAILURE_REASONS: Readonly<Record<string, FailureReason>> = Object.freeze({
  // ---- B2C ResultCodes (complete per Daraja B2C documentation) -------------
  ...
  // ---- M-PESA core numeric errors (complete per Account Balance documentation) --
  '15': D(
    '15',
  ...
  '100000000': D(
    '100000000',
    'M-PESA has cached the request and is waiting to resend it — the payment may still complete',
    'AMBIGUOUS',
    'Do not resend. Wait for the callback or run a status query on this OriginatorConversationID',
    true,
  ),
  '100000001': D('100000001', 'M-PESA reports the system is overloaded', 'PROVIDER', 'The reconciliation sweep will re-check', true),
  '100000002': D('100000002', 'M-PESA throttling error', 'PROVIDER', 'Throughput is throttled automatically; the sweep will re-check', true),
  '100000004': D('100000004', 'M-PESA internal server error', 'PROVIDER', 'Reconcile before any retry', true),
  '100000005': D(
    '100000005',
    'M-PESA rejected an input value as invalid',
    'REQUEST',
    'Check the instruction fields (amount, MSISDN, remarks); raise an engineering incident with the correlation id if they look correct',
    false,
  ),
  '100000007': D(
    '100000007',
    'The M-PESA service status is abnormal',
    'PROVIDER',
    'Hold releases until Safaricom reports the service healthy; the reconciliation sweep will resolve in-flight payments',
    true,
  ),
  '100000009': D(
    '100000009',
    'The API status is abnormal',
    'PROVIDER',
    'Hold releases until Safaricom reports the API healthy; the reconciliation sweep will resolve in-flight payments',
    true,
  ),
  '100000010': D(
    '100000010',
  ...
  // ---- Gateway / platform error codes (complete) -----------------------------
  '400.002.02': D(
    '400.002.02',
  ...
  '401.001': D(
    '401.001',
    'The Daraja access token was rejected for this API product',
    'CREDENTIAL',
  ...
  '404.001.04': D(
    ...
    'REQUEST',
    'Raise an engineering incident with the correlation id — all M-PESA calls are POST except token generation',
    false,
  ),
  '404.003.01': D(
    '404.003.01',
    'Daraja reports the endpoint does not exist for this API product',
    'REQUEST',
    'Raise an engineering incident — the endpoint path or the app product assignment is wrong',
    false,
  ),
  '405.001': D(
    '405.001',
    'Method not allowed — Daraja expected a different HTTP verb',
    'REQUEST',
    'Raise an engineering incident with the correlation id',
    false,
  ),
  ...
  SLV_RETRY_ISSUED: D(
    'SLV_RETRY_ISSUED',
    'This transaction is the retry of an earlier failed instruction',
    'UNKNOWN',
    'See the original transaction for the underlying failure reason',
    false,
  ),
});
...
/** One database-backed override entry. */
export interface FailureOverrideEntry {
  reason: string;
  class: FailureClass;
  operatorAction: string;
  transient: boolean;
...
export type FailureOverrides = Readonly<Record<string, FailureOverrideEntry>>;
 ...
 * and never returns the word "Error" alone.
 */
export function resolveFailure(
  ...
  return {
    ...
    failureClass: 'UNKNOWN',
    operatorAction:
      'Record the outcome with Safaricom support, then add a mapping under Settings → Failure reasons so future occurrences are explained automatically',
    transient: false,
    mapped: false,
    dictionaryVersion: FAILURE_DICTIONARY_VERSION,
  };
