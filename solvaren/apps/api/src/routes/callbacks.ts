/**
 * Daraja callback ingress (spec §9.4) — the most exposed surface SOLVAREN has, and
 * therefore deliberately the *dumbest* route: authenticate, persist the raw body,
 * enqueue, return 200. It makes no state decisions, touches no ledger row, and cannot
 * create a transaction.
 *
 * Defences, in order of application:
 *
 *  1. **Path-scoped organisation + secret.** The organisation AND the shared secret come
 *     from the URL — Safaricom POSTs to the ResultURL exactly as configured and sends no
 *     custom headers, so the URL itself carries the authentication material. A forged
 *     body cannot direct a result at another tenant.
 *  2. **Constant-time comparison** of the presented secret. A wrong secret returns 200 —
 ...
 *     recorded as duplicates instead of reprocessed.
 *  4. **Size limit before parse**, so an oversized body cannot exhaust the process.
 *  5. **Always 200.** Daraja does not retry, so a non-200 from us loses the result
 *     permanently; the body is already stored, and our own processing failure is our
 *     problem to fix.
 *  6. **Rate-limited by source IP** — unauthenticated DB-backed work must not be free.
 */
...
async function ingest(c: Context<AppContext>, kind: CallbackKind): Promise<Response> {
  ...
  const presentedSecret = c.req.param('secret') ?? '';
  ...
  await withConnection(c.env, async (sql) => {
    ...
    const authenticated =
      (expectedSecret !== null && presentedSecret !== '' && timingSafeEqual(presentedSecret, expectedSecret)) ||
      (globalSecret !== undefined && presentedSecret !== '' && timingSafeEqual(presentedSecret, globalSecret));
    ...
    if (!authenticated) {
      ...
      await inTransaction(sql, async (tx) => {
        ...
        `.catch(() => {
          // An unknown organisation id has no row to attach the event to.
        });
      });
...
callbackRoutes.post(
  '/daraja/callbacks/:organizationId/:secret',
  rateLimit('ip', { ratePerSecond: 20, burst: 100 }),
  (c) => ingest(c, 'B2C_RESULT'),
);
callbackRoutes.post(
  '/daraja/callbacks/timeout/:organizationId/:secret',
  rateLimit('ip', { ratePerSecond: 20, burst: 100 }),
  (c) => ingest(c, 'B2C_TIMEOUT'),
);
callbackRoutes.post(
  '/daraja/callbacks/status/:organizationId/:secret',
  rateLimit('ip', { ratePerSecond: 20, burst: 100 }),
  (c) => ingest(c, 'TRANSACTION_STATUS'),
);
callbackRoutes.post(
  '/daraja/callbacks/balance/:organizationId/:secret',
  rateLimit('ip', { ratePerSecond: 20, burst: 100 }),
  (c) => ingest(c, 'ACCOUNT_BALANCE'),
);
