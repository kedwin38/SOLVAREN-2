 ...
 *
 *   AI CONTROL — "The AI layer must never directly issue a payment release command. AI
 *   output is advisory unless converted into an explicit, human-authorized, signed action
 *   through the standard chain."
 *
 * This file makes that constraint structural rather than aspirational:
 *
 *  - it is a **read-only** route group. No handler writes to any payment table, and the
 *    only INSERT is into `ai_interactions`, whose `caused_state_change` column is pinned
 *    to FALSE by a CHECK constraint;
 *  - the model is given *derived summaries*, never the ability to call back into
 *    SOLVAREN. There are no tools, no function calling, no agentic loop;
 *  - recipient identifiers are masked before they are sent anywhere, so a payroll does
 *    not leave the execution environment in identifiable form;
 *  - without `AI_API_KEY` the layer is simply off, and the deterministic risk engine and
 *    failure dictionary are unaffected — the intended posture for an organisation whose
 *    data may not leave its environment.
 */
...
async function recordInteraction(
  ...
  await withConnection({ sql: env.sql }, async (sql) => {
    await sql`
      INSERT INTO ai_interactions (
        organization_id, user_id, capability, prompt_summary, context_digest,
        context_row_count, model, response_summary, caused_state_change, degraded, latency_ms
      ) VALUES (
        ...
        ${responseSummary.slice(0, 2000)}, FALSE, ${result.degraded}, ${result.latencyMs}
      )
    `;
  });
}
...
/**
 * POST /api/ai/batches/:id/analyse — explain a batch and its risk findings.
 *
...
/** POST /api/ai/failures/explain — explain a failure in plain language (§11.3). */
...
/** POST /api/ai/analysis/expenditure — compare payroll cycles and explain movements (L2/L3). */
...
/** POST /api/ai/briefing — the L3 executive briefing draft (§4.3, §12.2). */
...
Write four or five sentences in the style of a board briefing: the movement, its main driver, the risk position, and anything outstanding. State figures precisely.`;
  ...
  return c.json({
    ...
    degraded: result.degraded,
    // Computed facts, distinguished from AI narrative (spec §12.2).
    figures: {
