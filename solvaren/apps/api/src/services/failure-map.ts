/**
 * Failure-reason overrides (spec §6, TRK-009).
 *
 * Resolution order: organisation override → platform default row → the compiled
...
import type { FailureClass, FailureOverrideEntry } from '@solvaren/core';
...
): Promise<Record<string, FailureOverrideEntry>> {
  try {
    ...
    const overrides: Record<string, FailureOverrideEntry> = {};
    ...
    // A dictionary lookup failure must never stop a failure being explained; the
    // compiled dictionary takes over.
    console.warn(
      JSON.stringify({
        level: 'warn',
