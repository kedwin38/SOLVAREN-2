 ...
 *
 * Argon2id is required by spec §7.2. `hash-wasm` is used rather than a native binding so
 * the identical implementation runs in development, CI and production, and the hashes the
 * test suite verifies are the hashes production verifies. Parameters follow OWASP 2024
...
/**
 * The Frontier Payment Authorization Credential — the PIN half (spec §7.4 FPAC).
 *
 * The PIN is hashed with the same Argon2id cost as a password despite being shorter,
 * because its short length is exactly why it needs the work factor. It is additionally
 * bound to the user id: a PIN hash lifted from the database cannot be replayed against a
 * different account. The device-backed half of the FPAC is the WebAuthn assertion over
 * the manifest digest, verified in the release route.
 */
...
export function assertPinShape(pin: string): void {
  ...
  if (ascending || descending) {
    throw validationError('PIN_SEQUENTIAL', 'The authorization PIN must not be a sequence of consecutive digits');
  }
}
...
/**
 * HMAC-SHA-256 of a session token under the service's signing key.
 *
 * A plain SHA-256 would let anyone who dumps the sessions table mount an offline search
...
/** Constant-time comparison for any place a secret is compared to user input. */
export function timingSafeEqual(a: string, b: string): boolean {
  const aBytes = encoder.encode(a);
  const bBytes = encoder.encode(b);
  // The loop always runs the same number of iterations, and the length difference is
...
/**
 * Envelope-encrypt an organisation secret (Daraja credentials, backup keys, webhook
 * targets) with AES-GCM under a key derived from the master encryption key with a
 * per-purpose label (domain separation — a key derived for one purpose cannot be
 * confused into another).
 *
 * The IV is random per encryption and prefixed to the ciphertext. Envelopes carry a key
 * version so the master key can be rotated without a big-bang re-encryption: old
 * envelopes decrypt under their labelled version until re-wrapped.
 */
export const SECRET_ENVELOPE_VERSION = 1;

async function deriveKey(masterKey: string, purpose: string, version: number): Promise<CryptoKey> {
  const keyMaterial = await crypto.subtle.digest(
    'SHA-256',
    encoder.encode(`${masterKey}\x1f${purpose}\x1f${version}`),
  );
...
export async function encryptSecret(
  ...
  const key = await deriveKey(masterKey, purpose, SECRET_ENVELOPE_VERSION);
  ...
  combined[0] = SECRET_ENVELOPE_VERSION;
  combined.set(iv, 1);
  combined.set(new Uint8Array(ciphertext), 1 + iv.length);
  return toBase64Url(combined);
}

export async function decryptSecret(envelope: string, masterKey: string, purpose = 'secret'): Promise<string> {
  ...
  const key = await deriveKey(masterKey, purpose, version);
  try {
    const plaintext = await crypto.subtle.decrypt(
      { name: 'AES-GCM', iv: combined.slice(1, 13) },
      key,
      combined.slice(13),
    );
