 ...
 * signature over the exact manifest, and the FPAC PIN — see `services/authorization.ts`.
 *
 * There is no SMS path here, and nowhere to add one: the schema has no phone column on
 * any identity table, and the invariant checker fails CI if one appears.
 */
...
/** Access sessions last a working day; refresh happens by re-authenticating. */
export const SESSION_TTL_MS = 12 * 60 * 60 * 1000;
/**
 * Privileged actions require authentication no older than this (spec §8.2). The
 * step-up endpoint (`POST /api/auth/step-up`) refreshes this timestamp — a gate without
 * a relief valve would make every release impossible five minutes after login.
 */
 ...
 * an attacker a reliable account-enumeration oracle. This hash is never a valid
 * credential: it is the encoding of a random 32-byte value no one holds.
 */
const DUMMY_HASH =
  '$argon2id$v=19$m=19456,t=2,p=1$c29sdmFyZW5kdW1teXNhbHQ$c29sdmFyZW5kdW1teXdoYXNodmFsdWUwMDAwMDA';
 ...
 * Distinguishing them would tell an attacker which emails are real and which officers
...
export async function verifyPasswordStage(
  ...
  return {
    user,
    requiresWebAuthn: user.authority_level !== 'L1',
  };
}
...
export async function issueSession(
  ...
  const tokenHash = await hashSessionToken(token, signingKey);
  const now = new Date();
...
export type EffectiveMatrixCarrier = { matrix: EffectiveMatrix };

/**
 * Resolve a bearer token to an actor.
 *
 * Every check runs on every authenticated request, because each corresponds to something
 * that can change between login and now: the session can be revoked, the account can be
 * disabled or demoted, the device can be blocked.
 */
 ...
 * session being used to act hours after the officer walked away. The step-up endpoint
 * exists precisely so this gate can be satisfied mid-session without a full re-login.
 */
...
/** Revoke every session for a user — device revocation, compromise response, lockout. */
