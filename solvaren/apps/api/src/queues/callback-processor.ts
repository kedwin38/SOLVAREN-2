/**
 * Provider callback processing (spec §9.4).
 *
 ...
 *
 * Duplicate handling is explicit. Daraja can re-deliver, and a replayed callback is also
 * an attack (spec §24: "submit duplicate callback payloads and confirm transaction state
 * remains correct"). Each payload is deduplicated by digest at ingress, and a callback
 * that agrees with an already-settled transaction is recorded as a duplicate rather than
 * reapplied. A callback that *contradicts* a settled transaction opens a discrepancy
 * case for a human — the ledger is never rewritten.
 */
...
async function applyB2cResult(
  ...
  await inTransaction(sql, async (tx) => {
    ...
    if (!transaction) {
      ...
      // It is never used to create a transaction: that would let anyone who can reach
      // the endpoint invent a payment record.
      await tx`
...
async function applyTransactionStatus(
  ...
  await inTransaction(sql, async (tx) => {
    ...
    // discrepancy case (spec §14: historical transaction records are not alterable).
    if (['SUCCESS', 'FAILED', 'CANCELLED'].includes(transaction.status)) {
      const agrees =
    ...
    await tx`
      UPDATE transactions
         SET status = ${targetStatus},
             ...
             completed_at = ${parsed.finalisedTime ?? new Date().toISOString()}
       WHERE id = ${transaction.id}
    `;
    await tx`
      UPDATE payment_instructions SET status = ${targetStatus} WHERE id = ${transaction.instruction_id}
...
/** Apply an Account Balance callback for the L3 executive panel. */
