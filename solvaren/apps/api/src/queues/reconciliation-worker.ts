/**
 * Reconciliation and status sweeps (spec §9.3, §23).
 *
 * This worker resolves the one question the payment executor deliberately refuses to
 * guess at: for a transaction whose outcome we never learned, did money move?
 *
 ...
 *
 * An escalated case is not a resolved one: the transaction stays visible as unresolved,
 * because "we stopped asking" is not an outcome.
 */
 ...
 * bounded per run so one organisation with a large backlog cannot exhaust the Daraja
 * rate budget for everyone.
 */
  ...
  for (const transaction of stuck) {
    await sql`
      INSERT INTO reconciliation_cases (
        ...
        ${'REC-' + transaction.originator_conversation_id.slice(-10).toUpperCase()}, 'OPEN',
...
/** On-demand refresh for one transaction, or the bounded backlog of unresolved ones. */
export async function reconcileTransaction(
  sql: Sql,
  env: Env,
  message: Extract<ReconciliationQueueMessage, { type: 'RECONCILE_TRANSACTION' }>,
): Promise<void> {
...
async function queryTransactionStatus(
  ...
  const config = await sql<
    { short_code: string; initiator_name: string; status_result_url: string; queue_timeout_url: string }[]
  >`
    SELECT short_code, initiator_name, status_result_url, queue_timeout_url
  ...
  try {
    await client.queryTransactionStatus({
      ...
      IdentifierType: '4',
      // Status results arrive on the dedicated status callback URL (no string surgery).
      ResultURL: config[0].status_result_url,
    ...
    await sql`
      UPDATE reconciliation_cases
         SET query_attempts = query_attempts + 1,
             next_query_at = now() + (interval '10 minutes' * GREATEST(1, query_attempts)),
...
/** Hand an unresolvable case to a human, loudly — and notify. */
  ...
  await inTransaction(sql, async (tx) => {
    ...
    await tx`
      ...
      VALUES (
        ...
        ${'Reconciliation case escalated'},
        ${`A transaction outcome could not be established automatically after ${attempts} status queries and needs manual resolution.`},
        ${'/reconciliation'}
      )
    `;
...
/**
 * Refresh the organisation M-PESA balance for the L3 executive panel.
 *
 * Asynchronous like everything else on Daraja: this asks, and the callback answers. The
 * panel always shows an "as of" timestamp and an awaiting-refresh state rather than
 * pretending the figure is live.
 */
export async function refreshAccountBalance(
  sql: Sql,
  env: Env,
  message: Extract<ReconciliationQueueMessage, { type: 'REFRESH_BALANCE' }>,
): Promise<void> {
