 ...
 *     writes a QUEUED row first, and the terminal status is only ever set from what
 ...
 *   - **"Retention deletion failure → record partial; do not claim complete" (§23).**
 *     Retention runs only after a new backup is durably committed, and
 *     `retention_complete` stays false unless every deletion succeeded.
 *   - **Asynchronous and observable (§13.3).** The browser request that asks for a
 *     backup returns immediately with an attempt reference; this worker does the work.
 *
 * The snapshot is a logical JSON export of the organisation's rows under REPEATABLE READ
 * — point-in-time consistent on any managed PostgreSQL, including hosts that do not
 * expose pg_dump to the application. Credential hashes and sessions are deliberately
 * excluded: a restore must not resurrect live sessions, and password material has no
 * business travelling to object storage even hashed.
 */

import { reference } from '@solvaren/core';
 ...
 *
 * Session tokens and credential hashes are not restored from a backup, and secret
 * *references* are included because they are only names — the envelopes themselves live
 * in the secret store, which is itself backed up under the secrets/ prefix.
 */
...
export async function runBackup(sql: Sql, env: Env, message: BackupQueueMessage): Promise<void> {
  ...
  >`
    SELECT id, bucket, path_prefix, provider_label, retention_max_count,
  ...
  const attemptReference = message.attemptId ?? reference('backup');
  const targetDescription = config
    ? `${config.provider_label} ${config.bucket}/${config.path_prefix}`
    : 'unconfigured';

  if (!config) {
    await recordFailure(sql, message, attemptReference, targetDescription, 'BACKUP_NOT_CONFIGURED', 'No backup target is configured for this organisation');
    return;
  }

  // Spec §23: stop hammering a target whose credentials are rejected.
  ...
  const attemptRows = await sql<{ id: string }[]>`
    INSERT INTO backup_attempts (
      ...
      ${message.organizationId}, ${attemptReference}, ${message.trigger}, 'RUNNING', ${startedAt},
  ...
  try {
      ...
      await tx`
        UPDATE backup_attempts
           SET status = 'SUCCESS', ended_at = now(), object_key = ${objectKey},
               ...
               snapshot_method = 'logical-json-v1',
               snapshot_identifier = ${objectKey}
         WHERE id = ${attemptId}
      `;
      await tx`
    ...
    await inTransaction(sql, async (tx) => {
      ...
      await tx`
        ...
        VALUES (
          ${message.organizationId}, 'WARNING',
          ${'Backup failed'},
          ${`Offsite backup ${attemptReference} failed: ${messageText}`},
          ${'/settings/backups'}
        )
      `;
      await writeAuditEvent(tx, {
 ...
 *
 * Oldest first, and only ever beyond the limit. If any deletion fails, the run is
 * recorded as partial — spec §23: "do not claim retention complete."
 */
export async function enforceRetention(
  ...
  for (const backup of surplus) {
    try {
      ...
      // The attempt row is never deleted — it is the evidence that the backup existed
      // and was retired. Only a marker is added.
      await sql`
        UPDATE backup_attempts
 ...
 * snapshot of the database, so the transactions table cannot contain a row whose batch
 * is missing because it was created between two SELECTs.
 */
async function buildLogicalSnapshot(sql: Sql, organizationId: string): Promise<Snapshot> {
  ...
  await sql.begin(async (tx) => {
    ...
    for (const table of SNAPSHOT_TABLES) {
      ...
      // `organizations` is keyed by id rather than organization_id; most other tables
      // are tenant-scoped, so a snapshot contains exactly one organisation's data.
      const rows =
        table === 'organizations'
          ...
          : table === 'batch_editors'
            ...
            : table === 'batch_template_items'
              ? await tx.unsafe(
                  `SELECT bti.* FROM batch_template_items bti
                     JOIN batch_templates t ON t.id = bti.template_id
              ...
              : table === 'failure_reason_map'
                ...
                : table === 'security_events'
                  ? await tx.unsafe(
                      `SELECT * FROM security_events WHERE organization_id = $1 OR organization_id IS NULL`,
                      [organizationId],
                    )
  ...
  return {
    metadata: {
      ...
      note:
        ...
        'Secret envelopes live in the secrets/ prefix of the same bucket. ' +
