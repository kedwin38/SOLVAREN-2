 ...
 *
 *   - we know it was accepted  → record AWAITING_CALLBACK, wait for the result;
 ...
 * employee twice; a worker that gives up leaves them unpaid. Neither is acceptable, so
 * the outcome is established by querying the Transaction Status API, not by assumption
 * (spec §9.3, §23).
 */
 ...
 *
 * The idempotency claim transitions to SUBMITTED and commits *before* the HTTP call.
 * That ordering is deliberate and is the difference between a recoverable and an
 * unrecoverable failure: if the worker dies mid-call, the committed claim tells the
 * next worker "a request may have reached M-PESA — go and find out", instead of
 * "nothing happened, send it again".
 */
export async function executeInstruction(
  ...
  const { client, credentials, config, request, transactionId, instruction } = context;
  ...
  const ambiguous =
    ...
    providerCode === '100000000' || // request cached, waiting for resend
...
interface SubmissionContext {
  skip: boolean;
  client: import('@solvaren/daraja').DarajaClient;
  credentials: import('@solvaren/daraja').DarajaCredentials;
  config: Awaited<ReturnType<typeof loadDarajaClient>>['config'];
  request: {
    OriginatorConversationID: string;
    InitiatorName: string;
    SecurityCredential: string;
    CommandID: import('@solvaren/daraja').B2cCommandId;
    Amount: string;
    PartyA: string;
    PartyB: string;
    Remarks: string;
...
/**
 * Verify the message against live state, claim idempotency, and create the transaction
 * row. Committed before the provider call, so the claim survives a worker death.
 */
async function prepareSubmission(
  ...
  const prepared = await inTransaction(sql, async (tx) => {
    ...
    // The message carries the batch version it was authorized under. If the batch has
    // been edited since, this message describes a payment that is no longer authorized.
  ...
  // One cached client per configuration: the token cache serves every submission.
 ...
 * "success" while three people went unpaid is the exact invisibility the spec forbids.
 */
